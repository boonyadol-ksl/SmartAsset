...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'get_cost_centers') {
        ...
        $params = [];
        if ($pCode !== '') { $query .= " AND a.plant_code = ?"; $params[] = $pCode; }
        if ($userRole === 'webadmin' || $userRole === 'inventory') { $query .= " AND aa.user_id = ?"; $params[] = $userId; }
    ...
    if ($_GET['action'] === 'search_by_qr') {
        ...
        if ($assetId <= 0 && $prefix === '') { echo json_encode(['count' => 0, 'items' => [], 'prefix' => '']); exit; }
        ...
        if ($userRole === 'webadmin' || $userRole === 'inventory') { $qrWhere[] = 'aa.user_id = ?'; $qrParams[] = $userId; }
        $qrWhereStr = implode(' AND ', $qrWhere);

        $results = $db->fetchAll(
            "SELECT aa.id as assignment_id, aa.status as assignment_status, aa.checked_at,
                    a.asset_no, a.asset_description, a.serial_no, a.plant_code, a.cost_center
             FROM audit_assignments aa
             JOIN assets a ON aa.asset_id = a.id
             WHERE $qrWhereStr
    ...
    if ($_GET['action'] === 'get_audit') {
        ...
        $row = $db->fetchOne(
            "SELECT aa.*, a.asset_no, a.asset_description
             FROM audit_assignments aa JOIN assets a ON aa.asset_id = a.id
             WHERE aa.id = ?",
            [$assignId]
        );
        if (!$row) { echo json_encode(['success' => false, 'message' => 'ไม่พบรายการ']); exit; }
        if ($userRole !== 'admin' && $row['user_id'] != $userId) { echo json_encode(['success' => false, 'message' => 'ไม่มีสิทธิ์']); exit; }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_GET['action'] === 'save_audit') {
        ...
        if (!$assignId) { echo json_encode(['success' => false, 'message' => 'ไม่พบรายการ']); exit; }

        $assign = $db->fetchOne("SELECT * FROM audit_assignments WHERE id = ?", [$assignId]);
        if (!$assign) { echo json_encode(['success' => false, 'message' => 'ไม่พบรายการ']); exit; }
        if ($userRole !== 'admin' && $assign['user_id'] != $userId) {
            echo json_encode(['success' => false, 'message' => 'ไม่มีสิทธิ์ดำเนินการ']); exit;
        }
...
$where  = ['1=1'];
$params = [];
if ($userRole === 'webadmin' || $userRole === 'inventory') { $where[] = 'aa.user_id = ?'; $params[] = $userId; }
if ($search !== '') {
...
if ($auditorId  !== '') { $where[] = 'aa.user_id = ?';    $params[] = $auditorId; }
if ($status     !== '') { $where[] = 'aa.status = ?';     $params[] = $status; }
$whereStr = implode(' AND ', $where);

$total       = $db->fetchOne("SELECT COUNT(*) as c FROM audit_assignments aa JOIN assets a ON aa.asset_id = a.id WHERE $whereStr", $params)['c'];
$assignments = $db->fetchAll(
    "SELECT aa.id as assignment_id, aa.status as assignment_status, aa.checked_at, aa.remark as assignment_remark,
            aa.user_id as assigned_user_id, a.*, u.full_name as auditor_name
     FROM audit_assignments aa
     JOIN assets a ON aa.asset_id = a.id
     LEFT JOIN users u ON aa.user_id = u.id
     WHERE $whereStr ORDER BY a.asset_no ASC LIMIT $limit OFFSET $offset",
    $params
);
...
$statsWhere  = ['1=1'];
$statsParams = [];
if ($userRole === 'webadmin' || $userRole === 'inventory') { $statsWhere[] = 'aa.user_id = ?'; $statsParams[] = $userId; }
$statsWhereStr = implode(' AND ', $statsWhere);
$statsRaw  = $db->fetchAll(
    "SELECT a.plant_code, a.cost_center, aa.status, COUNT(*) as qty
     FROM audit_assignments aa JOIN assets a ON aa.asset_id = a.id
     WHERE $statsWhereStr GROUP BY a.plant_code, a.cost_center, aa.status",
    $statsParams
);
...
foreach ($statsRaw as $row) {
    ...
    if (!isset($summary['breakdown'][$pKey][$cKey]))
        $summary['breakdown'][$pKey][$cKey] = ['total' => 0, 'completed' => 0, 'pending' => 0];
    $qty = (int)$row['qty'];
    $summary['total'] += $qty;
    $summary['breakdown'][$pKey][$cKey]['total'] += $qty;
    if ($row['status'] === 'completed') { $summary['completed'] += $qty; $summary['breakdown'][$pKey][$cKey]['completed'] += $qty; }
    else { $summary['pending'] += $qty; $summary['breakdown'][$pKey][$cKey]['pending'] += $qty; }
}
...
$plantDropdownParams = [];
if ($userRole === 'webadmin' || $userRole === 'inventory') { $plantDropdownSql .= " AND aa.user_id = ?"; $plantDropdownParams[] = $userId; }
...
if ($userRole === 'webadmin' || $userRole === 'inventory') { $auditorDropdownSql .= " AND aa.user_id = ?"; $auditorDropdownParams[] = $userId; }
...
<style>
    a, .sidebar a, .main-content a { text-decoration: none !important; }
    .btn-check:checked + .btn { background-color: var(--bs-btn-active-bg); color: #fff; }
    ...
    #signature-canvas:active { border-color: #0d6efd; }

    /* Image preview cards */
    .img-preview-card { width: 100px; height: 82px; cursor: pointer; transition: transform .15s; }
    .img-preview-card:hover { transform: scale(1.05); }

    /* Toast */
    #toast-container { position: fixed; top: 16px; right: 16px; z-index: 9999; min-width: 240px; }

    @media (max-width: 767px) {
        #toast-container { right: 8px; left: 8px; width: auto; min-width: 0; }
        .img-preview-card { width: 72px; height: 72px; }
        .table-responsive { overflow-x: auto; }
        .table-responsive table { width: 100%; min-width: 640px; }
        .table-responsive td, .table-responsive th { white-space: normal; }
        #qr-reader { min-height: 200px; }
        #signature-canvas { height: 120px; }
    }
</style>

<div class="min-h-screen flex-1 p-4 sm:p-6">
    <?php include __DIR__ . '/../components/sidebar.php'; ?>

    <main class="main-content flex-1 p-4 sm:p-6 lg:ml-72 transition-all duration-300">
        ...
        <!-- Summary Dashboard -->
        <div class="bg-white border border-blue-100 rounded-lg p-4 mb-5 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                <div class="bg-blue-50/70 border border-blue-200 rounded p-3 text-center">
                ...
                <div class="bg-green-50/70 border border-green-200 rounded p-3 text-center">
                ...
                <div class="bg-yellow-50/70 border border-yellow-200 rounded p-3 text-center">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-xs text-left text-gray-500">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white font-mono">
                            <?php if (empty($summary['breakdown'])): ?>
                                <tr><td colspan="5" class="p-3 text-center text-gray-400 font-sans">ไม่พบรายการ</td></tr>
                            <?php else: ?>
                                <?php foreach ($summary['breakdown'] as $pCode => $ccData): ?>
                                    <?php foreach ($ccData as $ccCode => $val): ?>
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="p-2.5 text-center font-semibold text-gray-900"><?= number_format($val['total']) ?></td>
                                            <td class="p-2.5 text-center text-green-600 font-semibold"><?= number_format($val['completed']) ?></td>
                                            <td class="p-2.5 text-center text-yellow-600 font-semibold"><?= number_format($val['pending']) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endforeach; ?>
                            <?php endif; ?>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
            <div class="overflow-x-auto table-responsive">
                <table class="min-w-full divide-y divide-gray-200 text-sm text-left text-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white text-xs">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-12 text-gray-400 font-sans">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <?php if ($canModify): ?>
                                    <button class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-[11px] font-medium inline-flex items-center gap-1 shadow-sm transition"
                                            onclick="openUpdateModal(<?= $a['assignment_id'] ?>, '<?= addslashes(htmlspecialchars($a['asset_no'])) ?>', '<?= addslashes(htmlspecialchars($a['asset_description'] ?? '')) ?>')">
            ...
            <div class="bg-gray-50 border-t border-gray-100 px-4 py-3 flex items-center justify-between text-xs text-gray-500">
                <div>แสดงแถวที่ <?= $offset + 1 ?> – <?= min($offset + $limit, $total) ?> จาก <?= number_format($total) ?> รายการ</div>
                <div class="flex gap-1 flex-wrap">
                    <?php if ($page > 1): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>" class="px-2 py-1 bg-white border border-gray-300 rounded hover:bg-gray-100 no-underline">ก่อนหน้า</a>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $i])) ?>"
                           class="px-2 py-1 rounded border no-underline <?= $i === $page ? 'bg-blue-600 text-white font-bold' : 'bg-white border-gray-300 hover:bg-gray-100' ?>"><?= $i ?></a>
                    <?php endfor; ?>
                    <?php if ($page < $totalPages): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page + 1])) ?>" class="px-2 py-1 bg-white border border-gray-300 rounded hover:bg-gray-100 no-underline">ถัดไป</a>
                    <?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
                ...
                <form id="auditForm" onsubmit="event.preventDefault();">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="file" class="form-control form-control-sm" id="upload-assets" accept="image/*" multiple>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold" style="font-size:11px;">
                            ...
                            <input type="file" class="form-control form-control-sm" id="upload-qrcodes" accept="image/*">
                        </div>
...
function fetchCostCentersAjax(plantCode, selectedCc = '') {
    ...
    fetch(`inventory.php?action=get_cost_centers&plant_code=${encodeURIComponent(plantCode)}`)
        ...
        .catch(() => { ccSelect.innerHTML = '<option value="">-- โหลดไม่สำเร็จ --</option>'; });
}
...
function startQRScanner() {
    ...
    try {
        if (typeof Html5Qrcode === 'undefined') {
            throw new Error('ไม่พบไลบรารี Html5Qrcode');
        }

        if (!window.isSecureContext) {
            throw new Error('เบราว์เซอร์อนุญาตให้เปิดกล้องเฉพาะ HTTPS หรือ localhost');
        }
        ...
        const startWithCamera = (cameraConfig) => html5QrCode.start(cameraConfig, config, onSuccess, onFrameError);

        startWithCamera({ facingMode: { ideal: 'environment' } }).catch(() => {
            return startWithCamera({ facingMode: 'environment' }).catch(() => startWithCamera({ facingMode: 'user' }));
        }).then(() => {
        ...
        const statusDiv = document.getElementById('qr-reader-status');
        if (statusDiv) {
            statusDiv.innerHTML = '<span class="text-danger"><i class="fa-solid fa-camera-slash me-1"></i>เกิดข้อผิดพลาด: ' + escHtml(String(err)) + '</span>';
        }
    }
}
    ...
    fetch(`inventory.php?action=search_by_qr&qr=${encodeURIComponent(qrText)}`)
        ...
        .then(data => {
            ...
            if (data.count === 1) {
                ...
                openUpdateModal(item.assignment_id, item.asset_no, item.asset_description);
            } else {
...
//  AUDIT MODAL
// ══════════════════════════════════════════════════════
function openUpdateModal(assignmentId, assetNo, assetDesc) {
    currentAssetId = assignmentId;
    document.getElementById('modal-asset-no').textContent   = assetNo;
    document.getElementById('modal-asset-desc').textContent = assetDesc;
...
function closeAuditModal() { auditBsModal.hide(); }
...
function loadExistingAuditData(assignmentId) {
    fetch(`inventory.php?action=get_audit&assignment_id=${assignmentId}`)
        ...
        .then(data => {
            ...
            if (rd.quantity !== undefined) document.getElementById('quantity').value = rd.quantity;
            if (rd.location) document.getElementById('location').value = rd.location;
            if (rd.notes)    document.getElementById('remarks').value  = rd.notes;

            // Load saved images
            assetsImagesList  = [];
            qrCodesImagesList = [];
            (data.images || []).forEach(img => {
                const entry = { base64: img.base64, saved: true, filename: img.filename };
    ...
    files.forEach(file => {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                if (imgAR > tgAR) { sw = img.height * tgAR; sx = (img.width - sw) / 2; }
                else              { sh = img.width / tgAR;  sy = (img.height - sh) / 2; }
                ...
                if (currentList.length < maxLimit) {
                    currentList.push({ base64: compressed, saved: false, filename: null });
                    renderPreviews();
                }
            };
...
function createPreviewCard(imgObj, type, idx) {
    ...
    `;
    // Fix full preview
    wrap.querySelector('img').onclick = function () { previewFullImage(this.dataset.fullsrc); };
    return wrap;
}
...
function saveAuditData() {
    ...
    // Collect all images
    const allImages = [
        ...assetsImagesList.map(img  => ({ type: 'assets',   base64: img.base64 })),
        ...qrCodesImagesList.map(img => ({ type: 'qr_codes', base64: img.base64 }))
    ];
