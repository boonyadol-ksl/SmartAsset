...
<button type="button" id="btn-deselect-all">Deselect All</button>
                        ...
                        <div class="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                            <label class="flex items-center cursor-pointer">
                                ...
                                <span class="text-xs text-blue-800 font-medium">สุ่มเฉพาะรายการที่ "ยังไม่เคยถูกตรวจ" ในปีปัจจุบัน</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="card p-6">
                        ...
                        <template x-if="setup.plant_code && userWorkload.length > 0">
                            <div class="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                <p class="text-[11px] font-bold text-gray-600 mb-2">📋 งานที่มีอยู่แล้วในปีนี้ (Plant: <span x-text="setup.plant_code"></span>)</p>
                                <div class="space-y-1">
...
<script>
    async function apiPost(url, data) {
        try {
            ...
            return {
                success: false,
                message: 'การเชื่อมต่อเซิร์ฟเวอร์ล้มเหลว'
            };
        }
    }
    ...
    function auditCreator() {
        return {
            ...
            async onPlantChange() {
                ...
                $sel.empty();
                if (!this.setup.plant_code) return;
                await Promise.all([
            ...
            async fetchCostCenters() {
                const res = await fetch(`<?= APP_URL ?>/api/audit_helper.php?action=get_cc&plant=${encodeURIComponent(this.setup.plant_code)}`)
                    .then(r => r.json()).catch(() => ({
                        success: false
                    }));
                ...
                $('#btn-select-all').on('click', function() {
                    const allValues = $('#cc-select option').map(function() {
                        return this.value;
                    }).get();
    ...
    // รอให้ dropdown render เสร็จก่อน
    setTimeout(() => {
        // ดึงค่าที่เลือกไว้แล้วทั้งหมด
        const selectedValues = $sel.val();
        // เข้าถึงรายการทั้งหมดใน dropdown
        $('.select2-results__option').each(function() {
            const $option = $(this);
            const optionValue = $option.attr('data-select2-id'); // หรือเช็คจาก attribute ที่คุณผูกไว้
            // หาว่า option นี้มีค่าตรงกับที่เลือกไว้ไหม
            // หมายเหตุ: ตรงนี้อาจต้องปรับ logic ตามโครงสร้าง data ของคุณ
            const data = $option.data('data');
            if (data && selectedValues.includes(data.id.toString())) {
                $option.hide(); // ซ่อนรายการที่เลือกแล้ว
            } else {
                $option.show(); // แสดงรายการที่ยังไม่เลือก
            }
        });
    }, 0);
});
                ...
                $sel.on('select2:select select2:unselect', function(e) {
                    $(this).select2('close'); // ปิด dropdown ให้คลิกเลือกต่อได้ง่าย
                    $(this).select2('open');
                });
                const self = this;
            ...
            async fetchUserWorkload() {
                if (!this.setup.plant_code) return;
                const res = await fetch(`<?= APP_URL ?>/api/audit_helper.php?action=get_user_workload&plant=${this.setup.plant_code}`)
                    .then(r => r.json()).catch(() => ({
                        success: false
                    }));
                    // console.log(res);
            ...
            async fetchCancelData() {
                ...
                const res = await fetch(`<?= APP_URL ?>/api/audit_helper.php?action=get_pending_assignments&plant=${encodeURIComponent(this.setup.plant_code)}`)
                    .then(r => r.json()).catch(() => ({
                        success: false
                    }));
            ...
            checkDuplicate() {
                ...
                // ตรวจสอบว่า CC ที่เลือกมี pending assign อยู่แล้วไหม
